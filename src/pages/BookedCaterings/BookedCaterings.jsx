...
import { fetchCaterings, fetchEvents } from "../../rtk/slices/adminSlice.js";

const BookedCaterings = () => {
  const { loading, caterings } = useSelector((slice) => slice.admin);
  ...
  if (loading) {
    return (
      <div className="flex flex-wrap gap-10 px-24 bg-zinc-800 text -white text-3xl min-h-screen items-center justify-center">
        loading.....
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-wrap gap-10 px-24 bg-zinc-300 min-h-screen">
      <h1 className="text-3xl text-center w-full py-4 mb-20">
        {" "}
        Booked Catreings
      </h1>
      {caterings.map((item, idx) => (
        ...
        >
          ...
          <div className="flex  gap-4 items-center">
            ...
            <h1 className="text-zinc-300">{item.email}</h1>
          </div>
          <div className="flex  gap-4 items-center">
            {" "}
